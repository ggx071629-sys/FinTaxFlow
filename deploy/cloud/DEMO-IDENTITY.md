...
`Dockerfile.update` overlays application code and portal assets onto the immutable previously verified API image (Python/Chrome dependency layer retained). Tag the existing image locally, verify the tag resolves to the expected SHA256 image ID, then supply that local tag as BASE_IMAGE and the Git commit as REVISION. COPY sets runtime ownership explicitly, so restricted archive extraction permissions cannot prevent the non-root API from reading code. Build H5 with `VITE_CLOUD_DEPLOYMENT=true VITE_API_BASE_URL=https://tax.gavingongxin.com/api npm run build:h5`; lockfile includes the prior Intlify/Vite fixes and .npmrc records the UniApp peer compatibility exception. Use the existing service topology, volumes and TLS configuration.
